...
After:
			...
			if (_handler.VirtualView is not PlatformView target)
			{
...
*/
			...
			// In XF this was called inside ViewDidLoad
			if (_handler?.ToPlatform() is View view)
			{
				OnElementChanged(this, new VisualElementChangedEventArgs(null, view));
			}
			else
...
After:
		static IList<GestureElement>? GetChildGestures(
			CGPoint originPoint,
			WeakReference weakEventTracker, WeakReference weakRecognizer, GesturePlatformManager? sender, System.ComponentModel.PropertyChangedEventArgs e)
*/
	return uiRecognizers;
			}
		...
		void OnTapGestureRecognizerPropertyChanged(object? eventTracker, View? view)
		{
			if (!weakRecognizer.IsAlive)
...
After:
		...
		UIPanGestureRecognizer CreatePanRecognizer(IGestureRecognizer recognizer)
*/
			{
				LoadRecognizers();
			}
		...
		ButtonsMask AddFakeRightClickForMacCatalyst(int numTouches, Action<UIPanGestureRecognizer> action)
		{
			var result = new UIPanGestureRecognizer(action);
			result.MinimumNumberOfTouches = result.MaximumNumberOfTouches = (uint)numTouches;
